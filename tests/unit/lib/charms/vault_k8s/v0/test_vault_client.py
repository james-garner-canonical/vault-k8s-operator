...
def test_when_destroy_autounseal_credentials_then_approle_and_policy_are_deleted():
    ...
    autounseal = VaultAutounsealManager(model, vault, tls_manager, provides, "ca_cert")

    autounseal.delete_credentials(relation)

    vault.delete_role.assert_called_with(f"charm-autounseal-{relation_id}")
    vault.delete_policy.assert_called_with(f"charm-autounseal-{relation_id}")
...
def test_when_create_autounseal_credentials_then_key_and_approle_and_policy_are_created():
    ...
    relation = MagicMock()
    relation.id = relation_id
    autounseal = VaultAutounsealManager(model, vault, provides, "ca_cert")
